...
namespace EbWeb.Models.Services.Application
{
    public class AdoNetRevisioneService : IRevisioneService
    {
        ...
        public async Task<ListViewModel<RevisioneViewModel>> GetRevisioniAsync(RevisioneListInputModel model)
        {
            ...
            FormattableString query = $@"SELECT DISTINCT Nag_Affidato, Intestazione, Info, Filtro FROM REV.Revisioni_Semplificate_BI WHERE NomeColonna = '02.05 - Revisione semplificata' AND Filtro LIKE {"%" + model.Search + "%"} ORDER BY {(Sql)orderby} {(Sql)direction} OFFSET {model.Offset} ROWS FETCH NEXT {model.Limit} ROWS ONLY;
