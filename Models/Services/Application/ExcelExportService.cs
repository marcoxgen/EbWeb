...
public class ExcelExportService : IExcelExportService
{
    public byte[] GenerateAbilitazioniExcel(IEnumerable<AbilitazioneMifidDetailViewModel> viewModel)
    {
        using (var workbook = new XLWorkbook())
        {
            ...
            worksheet.Cell(1, 3).Value = "Descrizio UO";
            ...
            worksheet.Cell(1, 26).Value = "Data Ultimo Aggiornamento";
            worksheet.Cell(1, 27).Value = "Genera Lettera X";
            worksheet.Cell(1, 28).Value = "Genera Lettera Y";
            worksheet.Cell(1, 29).Value = "Genera Lettera Z";
            ...
            foreach (var item in viewModel)
            {
                ...
                worksheet.Cell(currentRow, 9).Value = item.DataConseguimentoTitoloStudio?.ToString("yyyy-MM-dd");
                worksheet.Cell(currentRow, 10).Value = item.DataAbilitazioneMifid?.ToString("yyyy-MM-dd");
                worksheet.Cell(currentRow, 11).Value = item.DataSospensione?.ToString("yyyy-MM-dd");
                worksheet.Cell(currentRow, 12).Value = item.DataTermineSospensione?.ToString("yyyy-MM-dd");
                worksheet.Cell(currentRow, 13).Value = item.NecessarioAssessment == true ? "SI" : "NO";
                worksheet.Cell(currentRow, 14).Value = item.DataSuperamentoAssessment?.ToString("yyyy-MM-dd");
                worksheet.Cell(currentRow, 15).Value = item.DataAbilitazioneTitoli?.ToString("yyyy-MM-dd");
                worksheet.Cell(currentRow, 16).Value = item.AnniEsperienzaAdeguata;
                worksheet.Cell(currentRow, 17).Value = item.DataInizioSupervisione?.ToString("yyyy-MM-dd");
                worksheet.Cell(currentRow, 18).Value = item.DataFineSupervisione?.ToString("yyyy-MM-dd");
                ...
                worksheet.Cell(currentRow, 26).Value = item.DataUltimoAggiornamento?.ToString("yyyy-MM-dd");
                worksheet.Cell(currentRow, 27).Value = item.GeneraLetteraX;
                worksheet.Cell(currentRow, 28).Value = item.GeneraLetteraY;
                worksheet.Cell(currentRow, 29).Value = item.GeneraLetteraZ;
